...
$emails = mysql_query("SELECT * FROM wp_users WHERE DateExpire >= (DATE_ADD(curdate(), INTERVAL -2 MONTH)) && email1 != ''  ORDER BY wp_users . lastname", $connection);
...
<tr valign="top">
    ...
    <li>'.$newsRow["item1"].'</li>
    <li>'.$newsRow["item2"].'</li>';
    if ($newsRow["item3"] != '') {echo '<li>'.$newsRow["item3"].'</li>
    ';}
    echo '</ul>
