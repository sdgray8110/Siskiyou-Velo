<?php
 $File = "fullMemberHistory.csv";
 unlink($File);
 $Handle = fopen($File, 'a');
 $headers = array("Name","Member Type","City","State","email1","email2","Phone","Age","Date Joined","Expiration","Volunteer","Ride Leader","No Bicycles","City Bike","Road Bike","Mountain","Recumbent","Tandem","Fixie","No Bike","Socialize","Social Rides","Advocacy","Green Lifestyle","Riding Style","Riding Speed");
 fputcsv($Handle, $headers);

include('../includes/db_connect.php');

$query = mysql_query("SELECT * FROM wp_users ORDER BY wp_users . DateExpire ASC", $connection);
...
$memberstring =  array($name,$type,$row['city'],$row['state'],$row['email1'],$row['email2'],$row['phone'],$row['Age'],$joined,$expire,$volunteer,$rideLead,$nobike,$cityBike,$roadBike,$mountain,$recumbent,$tandem,$fixie,$nobike1,$socialize,$social_rides,$advocacy,$green,$style,$speed);
