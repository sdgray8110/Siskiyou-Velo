...
Simple:Press
Desc:
$LastChangedDate: 2012-05-20 10:19:33 -0700 (Sun, 20 May 2012) $
$Rev: 8553 $
*/
...
function sp_track_online() {
    ...
    if (!empty($topicId)) {
        ...
        if (!sp_get_auth('view_forum', $forumId) && !sp_get_auth('view_forum_topic_lists', $forumId)) return;
    }
	...
	if ($track) {
		...
		if ($spThisUser->member) sp_construct_users_newposts();
	}
	...
	if ($expired) {
		...
		foreach ($expired as $expire) {
			if ($expire->trackuserid > 0) {
				...
				sp_destroy_users_newposts($expire->trackuserid);
			}
		}
...
function sp_track_logout() {
	...
	sp_destroy_users_newposts($current_user->ID);
}
