...
$LastChangedDate: 2012-06-03 02:32:48 -0700 (Sun, 03 Jun 2012) $
$Rev: 8642 $
*/
...
function sp_GroupHeaderName($args='') {
	...
	$out = (empty($spThisGroup->group_name)) ? '' : "<div id='$tagId' class='$tagClass'>".sp_truncate($spThisGroup->group_name, $truncate)."</div>\n";
...
function sp_ForumIndexStatusIcons($args='', $toolTipLock='', $toolTipPost='', $toolTipAdd='') {
	...
	if($showNewPost) {
		if ($spThisForum->unread) {
			...
			$site = SFHOMEURL."index.php?sp_ahah=newpostpopup&amp;action=forum&amp;id=$spThisForum->forum_id&amp;sfnonce=".wp_create_nonce('forum-ahah');
