...
$LastChangedDate: 2012-05-13 16:13:06 -0700 (Sun, 13 May 2012) $
$Rev: 8510 $
*/
...
function sp_PostNewButton($args='', $label='', $toolTip='', $toolTipLock='') {
    ...
    if (!$lock && sp_get_auth('reply_topics', $spThisTopic->forum_id)) {
		$out.= "<a href='javascript:void(null)' class='$tagClass vtip' id='$tagId' title='$toolTip' onclick='jQuery(document).ready(function() { spjOpenEditor(\"spPostForm\", \"post\"); });'>\n";
		if (!empty($icon)) $out.= "<img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt=''/>\n";
		if (!empty($label)) $out.= sp_filter_title_display($label);
		$out.= "</a>\n";
	}
...
function sp_PostIndexPageLinks($args='', $label='', $toolTip='') {
	...
	if($spThisTopic->posts_per_page >= $spThisTopic->post_count) return '';
...
function sp_PostIndexUserRank($args='') {
	...
	if ($showBadge && !empty($spThisPostUser->rank[0]['badge'])) {
		...
		if (!empty($spThisPostUser->rank[0]['name'])) $tout.= "<br />";
	}
	if ($showTitle && !empty($spThisPostUser->rank[0]['name'])) {
	    $show = true;
		$tout.= '<span class="spRank-'.sp_create_slug($spThisPost->user->rank[0]['name'], 'rank', false).'">'.$spThisPostUser->rank[0]['name'].'</span>';
	}
	$tout.= "</div>\n";
	...
	if (($showBadge || $showTitle) && !empty($spThisPostUser->special_rank)) {
		foreach ($spThisPostUser->special_rank as $rank) {
			...
			if ($showTitle && !empty($rank['name'])) {
        	    $show = true;
				if (!empty($rank['badge'])) $tout.= '<br />';
				$tout.= '<span class="spSpecialRank-'.sp_create_slug($rank['name'], 'rank', false).'">'.$rank['name'].'</span>';
			}
		}
	}
	$tout.= "</div>\n";
...
function sp_PostIndexPinned($args='', $toolTip='') {
	...
	$out.= "<img class='vtip' src='$icon' title='$toolTip' alt='' />\n";
	$out.= "</span>";
...
function sp_PostIndexQuote($args='', $label='', $toolTip='') {
	...
	if ($spThisPostUser->member) {
		...
		$name = $spThisPostUser->guest_name;
	}
	$intro = $name.' '.sp_text('said').' ';

	$out = "<a class='$tagClass vtip' id='$tagId' title='$toolTip' rel='nofollow' href='javascript:void(null)' ";
	$out.= "onclick='jQuery(document).ready(function() { spjQuotePost($spThisPost->post_id, \"".esc_js($intro)."\", $spThisTopic->forum_id, \"$quoteUrl\"); });'>";
...
function sp_PostIndexContent($args='', $label='') {
	global $spThisTopic, $spThisPost, $spThisPostUser, $spThisUser;
	...
	if ($spThisPost->post_status == false) {
		...
		if(sp_get_auth('moderate_posts', $spThisTopic->forum_id)
			...
			|| ($spThisUser->guest && !empty($spThisUser->guest_email) && $spThisUser->guest_email == $spThisPostUser->guest_email)) {
...
function sp_PostIndexUserTwitter($args='', $toolTip='') {
	...
	if ($get) return $spThisPostUser->twitter;

    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='http://twitter.com/$spThisPostUser->twitter' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserFacebook($args='', $toolTip='') {
    ...
    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='http://facebook.com/$spThisPostUser->facebook' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserMySpace($args='', $toolTip='') {
    ...
    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='http://myspace.com/$spThisPostUser->myspace' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserLinkedIn($args='', $toolTip='') {
    ...
    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='http://linkedin.com/$spThisPostUser->linkedin' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserYouTube($args='', $toolTip='') {
    ...
    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='http://youtube.com/user/$spThisPostUser->youtube' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserGooglePlus($args='', $toolTip='') {
    ...
    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='https://plus.google.com/u/$spThisPostUser->googleplus/$spThisPostUser->googleplus' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserWebsite($args='', $toolTip='') {
    ...
    $target = ($targetNew) ? ' target="_blank"' : '';
	$out = "<a id='$tagId' class='$tagClass vtip' href='$spThisPostUser->user_url' title='$toolTip'$target><img class='$iconClass' src='".SPTHEMEICONSURL.$icon."' alt='' /></a>\n";
...
function sp_PostIndexUserStatus($args='', $onlineLabel='', $offlineLabel='') {
	...
	$icon = '';
	if (($spThisUser->admin ||
        (!$spMemberOpts['sfhidestatus'] || (!isset($spThisPostUser->user_options['hidestatus']) || !$spThisPostUser->user_options['hidestatus']))) &&
...
function sp_PostForumToolButton($args='', $label='', $toolTip='') {
	global $spThisTopic, $spThisPost, $spThisPostUser, $spThisUser, $sfglobals;
	...
	if ($spThisUser->admin || $spThisUser->moderator) {
		...
		if (sp_get_auth('view_email', $spThisTopic->forum_id) ||
			...
			(sp_get_auth('edit_own_posts_reply', $spThisTopic->forum_id) && $spThisUser->guest && $spThisPostUser->guest_email == $spThisUser->guest_email && $spThisPost->last_post) ||
