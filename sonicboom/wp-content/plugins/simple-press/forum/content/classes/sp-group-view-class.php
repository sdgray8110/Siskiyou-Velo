...
Simple:Press
Group View Class
$LastChangedDate: 2012-04-22 03:11:35 -0700 (Sun, 22 Apr 2012) $
$Rev: 8422 $
*/
...
class spGroupView {
	...
	function __construct($ids='', $stats=true) {
		...
		if (!empty($sfvars['groupid']) && !in_array($sfvars['groupid'], $gIds)) $gIds[] = $sfvars['groupid'];
	...
	function sp_groupview_query($groupids='') {
		...
		$spdb = apply_filters('sph_groupview_query', $spdb);
		$records = $spdb->select();
		...
		if ($records) {
			...
			foreach ($records as $r) {
				...
				if (sp_get_auth('view_forum', $forumid) || sp_get_auth('view_forum_topic_lists', $forumid) || sp_get_auth('view_forum_lists', $forumid)) {
					...
					foreach ($g as $gr) {
						foreach ($gr->forums as $f) {
							if (!empty($stats[$f->forum_id])) {
								...
								if (sp_get_auth('view_forum', $f->forum_id) && (sp_get_auth('view_admin_posts', $f->forum_id) || !sp_is_forum_admin($s->user_id))) {
