...
function sp_the_post() {
	...
	$spThisPostUser = $spThisPost->user;
...
class spTopicView {
	...
	function sp_topicview_query($topicid=0, $cPage=1, $forumid=0) {
		...
		$spdb = new spdbComplex;
			$spdb->table 	= SFTOPICS;
			$spdb->fields	= SFTOPICS.'.topic_id, '.SFTOPICS.'.forum_id, topic_name, topic_slug, topic_status, topic_icon, '.SFTOPICS.'.post_count, forum_slug, forum_status, forum_rss_private,
							  '.SFPOSTS.'.post_id, '.spdb_zone_datetime('post_date').', '.SFPOSTS.'.user_id,
							  guest_name, guest_email, post_status, post_pinned, post_index, post_edit, poster_ip, post_content'.$waitCheck;
		...
		$spdb = apply_filters('sph_topicview_query', $spdb);
		$records = $spdb->select();
		...
		if ($records) {
			...
			if (sp_get_auth('view_forum', $r->forum_id)) {
				...
				foreach ($records as $r) {
					...
					$t[$tidx]->posts[$pidx]->user				= $this->sp_topicview_get_user($r->user_id);
					...
					$t[$tidx]->posts[$pidx]->user->guest_name	= $t[$tidx]->posts[$pidx]->guest_name;
					$t[$tidx]->posts[$pidx]->user->guest_email	= $t[$tidx]->posts[$pidx]->guest_email;
				...
				if (sp_get_auth('view_forum_topic_lists', $r->forum_id)) $this->topicViewStatus = 'sneak peek';
			}
		}
		return $t;
	}

	function sp_topicview_get_user($uid) {
		global $spThisUser;
		if ($uid == $spThisUser->ID && $spThisUser->ID !=0 ) {
			return $spThisUser;
		} elseif(array_key_exists($uid, $this->userCache)) {
			return $this->userCache[$uid];
		} else {
			$this->userCache[$uid] = new spUser($uid);
			return $this->userCache[$uid];
		}
	}
}
