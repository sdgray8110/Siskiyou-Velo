...
class spForumView {
	...
	function sp_forumview_query($forumid=0, $cPage=1) {
		...
		$spdb = new spdbComplex;
			...
			$spdb->fields 	= SFTOPICS.'.forum_id, forum_slug, forum_name, forum_status, group_id, topic_count, forum_icon, topic_icon, topic_icon_new, forum_desc, forum_rss,
							forum_rss_private, parent, children, forum_message,
			...
			$spdb->orderby 	= $ORDER;
			$spdb->limits 	= $LIMIT;
		$spdb = apply_filters('sph_forumview_query', $spdb);
		$records = $spdb->select();
		...
		if ($records) {
			...
			if (sp_get_auth('view_forum', $forumid) || sp_get_auth('view_forum_topic_lists', $forumid)) {
				...
				foreach ($records as $r) {
					...
					if ($spThisUser->member && in_array($tidx, $spThisUser->newposts['topics'])) $f[$fidx]->topics[$tidx]->unread = 1;
				...
				if ($stats) {
					foreach ($stats as $s) {
						if ($s->post_index == 1) {
							...
							if (sp_get_auth('view_forum', $forumid) && (!sp_is_forum_admin($s->user_id) || sp_get_auth('view_admin_posts', $forumid))) {
						...
						if ($s->post_index > 1 || $f[$fidx]->topics[$s->topic_id]->post_count == 1) {
							...
							# see if we can display the tooltip
							if (sp_get_auth('view_forum', $forumid) && (sp_get_auth('view_admin_posts', $forumid) || !sp_is_forum_admin($s->user_id))) {
				...
				if (sp_get_auth('view_forum_lists', $forumid)) $this->forumViewStatus = 'sneak peek';
			}
		} else {
	...
	function sp_forumview_build_subforums($forumid, $f, $fidx, $subs) {
		...
		foreach ($subs as $sub) {
			if (sp_get_auth('view_forum', $sub->forum_id) || sp_get_auth('view_forum_topic_lists', $sub->forum_id)) {
			...
			if ($stats) {
				foreach ($subs as $sub) {
					if (!empty($stats[$sub->forum_id])) {
						...
						# see if we can display the tooltip
						if (sp_get_auth('view_forum', $sub->forum_id) && (sp_get_auth('view_admin_posts', $sub->forum_id) || !sp_is_forum_admin($s->user_id))) {
					...
					if($fsub != 0 && !empty($stats[$fsub])) {
						...
						# see if we can display the tooltip
						if (sp_get_auth('view_forum', $fsub) && (sp_get_auth('view_admin_posts', $fsub) || !sp_is_forum_admin($s->user_id))) {
	...
	function sp_forumview_subforums_query($subs) {
		...
		$spdb = apply_filters('sph_forumview_subforums_query', $spdb);
		$records = $spdb->select();
	...
	function sp_forumview_stats_query($topics, $forumid) {
		...
		$spdb = apply_filters('sph_forumview_stats_query', $spdb);
		$records = $spdb->select();
	...
	function sp_subforumview_stats_query($posts) {
		...
		$spdb = new spdbComplex;
			...
			$spdb->where		= $WHERE;
		$spdb = apply_filters('sph_groupview_stats_query', $spdb);
		$records = $spdb->select();
