...
Simple:Press
Desc:
$LastChangedDate: 2012-06-03 13:38:47 -0700 (Sun, 03 Jun 2012) $
$Rev: 8651 $
*/
...
function spa_admin_menu() {
	global $SPSTATUS, $sfadminpanels, $spThisUser;
	...
	if ($SPSTATUS == 'ok') {
            ...
            foreach ($sfadminpanels as $panel) {
                if ($panel[7] && ((sp_current_user_can($panel[1])) || ($panel[0] == 'Admins' && ($spThisUser->admin || $spThisUser->moderator)))) {
            		if (empty($adminparent)) {
                        $adminparent = $panel[2];
                        add_object_page('Simple:Press', esc_attr($panel[0]), 'read', $adminparent, '', 'div');
                    }
		...
		add_object_page('Simple:Press', esc_attr( spa_text('Forum')), 'activate_plugins', $parent, '', 'div');
...
function spa_setup_admin_menu() {
    ...
    */
	...
	$forms = array(
		...
		spa_text('Plugin Uploader') => array('upload' => ''));
	...
	$forms = array(
		...
		spa_text('Theme Uploader') => array('upload' => ''),
		spa_text('Theme Editor') => array('editor' => 'sfreloadttedit'));
