...
$LastChangedDate: 2011-12-28 15:29:40 -0700 (Wed, 28 Dec 2011) $
$Rev: 7601 $
*/
...
?>
						...
						<div>
							...
							<input type="button" class="sfform-panel-button" onclick="javascript:document.searchfilter.submit();" id="sfusersearch" name="sfusersearch" value="<?php spa_etext('Search Members'); ?>" />
			 			</div>
						<br class="clear" />
					</div>
					<br class="clear" />
				...
				<?php if ($user_search->get_results()) : ?>
					<?php if ($user_search->is_search()) : ?>
						<p><a href="<?php echo SFADMINUSER; ?>"><?php echo sprintf(spa_etext('%s Back to All Members'), '&laquo'); ?></a></p>
					<?php endif; ?>
...
<?php
							$style = '';
							$post_counts = count_many_users_posts($user_search->get_results());
...
<?php
    													...
    													if (current_user_can('delete_user', $userid)) {
    														...
    														if ($post_counts[$userid] > 0) {
    															...
    															echo '<a href="javascript:void(null)" onclick="spjDelRow(\''.$site.'\', \'user-delete-'.$userid.'\');"><img src="'.SFCOMMONIMAGES.'delete.png" title="'.spa_text('Delete User').'" alt="" /></a>';
    														}
    													}
?>
    												</td>
