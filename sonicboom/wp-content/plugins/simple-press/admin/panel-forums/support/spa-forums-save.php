...
$LastChangedDate: 2012-05-05 14:46:59 -0700 (Sat, 05 May 2012) $
$Rev: 8493 $
*/
...
function spa_save_forums_create_group() {
	...
	for ($x=0; $x<count($perm_list); $x++) {
		if ($perm_list[$x] == -1) {
			$mess = spa_text('All usergroups must be assigned a default permission');
			return $mess;
		}
	}
	...
	for( $x=0; $x<count($ug_list); $x++) {
		spa_add_defpermission_row($group_id, $ug_list[$x], $perm_list[$x]);
	}
...
function spa_save_forums_create_forum() {
	...
	if (!isset($_POST['thisforumslug']) || empty($_POST['thisforumslug'])) {
		$forumslug = sp_create_slug($forumdata['forum_name'], 'forum');
	} else {
		$forumslug = $_POST['thisforumslug'];
	}

	$sql = 'INSERT INTO '.SFFORUMS.' (forum_name, forum_slug, forum_desc, group_id, forum_status, forum_seq, forum_rss_private, forum_icon, forum_icon_new, topic_icon, topic_icon_new, parent, forum_message) ';
	$sql.= "VALUES ('".$forumdata['forum_name']."', '".$forumslug."', '".$forumdata['forum_desc']."', ".$forumdata['group_id'].", ".$forumdata['forum_status'].", ".$forumdata['forum_seq'].", ".$forumdata['forum_rss_private'].", '".$forumdata['forum_icon']."', '".$forumdata['forum_icon_new']."', '".$forumdata['topic_icon']."', '".$forumdata['topic_icon_new']."', ".$parentdata.", '".$forumdata['forum_message']."');";
...
function spa_save_forums_edit_forum() {
	...
	if (!empty($_POST['thisforumslug'])) {
		$forumdata['forum_slug'] = sp_create_slug($_POST['thisforumslug'], 'forum', false);
	} else {
		$forumdata['forum_slug'] = sp_create_slug($forumdata['forum_name'], 'forum');
	}
	$forumdata['forum_desc'] = sp_filter_text_save(trim($_POST['forum_desc']));
	...
	if($_POST['cforum_slug'] != $forumslug) {
		sp_update_post_urls($_POST['cforum_slug'], $forumslug);
	}
...
function spa_save_forums_edit_group() {
	...
	# save the default permissions for the group
	for ($x=0; $x<count($ug_list); $x++) {
		if (spa_get_defpermissions_role($group_id, $ug_list[$x])) {
