...
$LastChangedDate: 2012-05-26 11:27:48 -0700 (Sat, 26 May 2012) $
$Rev: 8574 $
*/
...
function spa_save_seo_data() {
    ...
    $sfmetatags['sfusekeywords'] = isset($_POST['sfusekeywords']);
...
function spa_update_specialrank($id) {
   check_admin_referer('special-rank-update', 'special-rank-update');
	...
	if (!empty($_POST['specialrankdesc'])) {
		...
		$rank[0]['meta_value']['badge'] = $badge[$id];
...
function spa_add_special_rank_member($id) {
	...
	for ($x=0; $x<count($user_id_list); $x++) {
        $memberData = sp_get_member_item($user_id_list[$x], 'special_ranks');
        $memberData[] = $rank[0]['meta_key'];
        sp_update_member_item($user_id_list[$x], 'special_ranks', $memberData);
	}
...
function spa_del_special_rank_member($id) {
	...
	for ($x=0; $x<count($user_id_list); $x++) {
        ...
        sp_update_member_item($user_id_list[$x], 'special_ranks', $newList);
	}
