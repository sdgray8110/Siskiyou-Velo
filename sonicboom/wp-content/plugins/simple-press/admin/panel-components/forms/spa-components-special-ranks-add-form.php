...
?>
		...
		<input type="submit" class="sfform-panel-button" id="sfmembernew<?php echo $rank_id; ?>" name="sfmembernew<?php echo $rank_id; ?>" onclick="javascript:jQuery('#amember_id<?php echo $rank_id; ?> option').each(function(i) {jQuery(this).attr('selected', 'selected');});" value="<?php spa_etext('Add Members'); ?>" />
		<input type="button" class="sfform-panel-button" onclick="spjToggleLayer('<?php echo $loc; ?>');javascript:jQuery('#members-<?php echo $rank_id; ?>').html('');" id="addmemberscancel<?php echo $rank_id; ?>" name="addmemberscancel<?php echo $rank_id; ?>" value="<?php spa_etext('Cancel'); ?>" />
		</div>
	</form>
