...
$LastChangedDate: 2011-09-11 16:23:30 -0700 (Sun, 11 Sep 2011) $
$Rev: 7076 $
*/
...
?>
		...
		<input type="submit" class="sfform-panel-button" id="sfmemberdel<?php echo $rank_id; ?>" name="sfmemberdel<?php echo $rank_id; ?>" onclick="javascript:jQuery('#dmember_id<?php echo $rank_id; ?> option').each(function(i) {jQuery(this).attr('selected', 'selected');});" value="<?php spa_etext('Remove Members'); ?>" />
		<input type="button" class="sfform-panel-button" onclick="spjToggleLayer('<?php echo $loc; ?>');javascript:jQuery('#members-<?php echo $rank_id; ?>').html('');" id="sfmemberdel<?php echo $rank_id; ?>" name="addmemberscancel<?php echo $rank_id; ?>" value="<?php spa_etext('Cancel'); ?>" />
		</div>
	</form>
