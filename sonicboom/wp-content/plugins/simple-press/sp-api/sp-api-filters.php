...
Simple:Press
Filters
$LastChangedDate: 2012-06-05 12:56:40 -0700 (Tue, 05 Jun 2012) $
$Rev: 8665 $
*/
...
function sp_filter_content_save($content, $action) {
	...
	$content = apply_filters('sph_save_post_content_filter', $content);
	return $content;
}
...
function sp_filter_text_save($content, $links=true) {
	$sffilters = sp_get_option('sffilters');
	...
	$content = apply_filters('sph_save_text_filter', $content);
...
function sp_filter_title_save($content) {
	...
	$content = apply_filters('sph_save_title_filter', $content);
...
function sp_filter_name_save($content) {
	...
	# 4: apply any users custom filters
	$content = apply_filters('sph_save_name_filter', $content);
...
function sp_filter_email_save($email) {
	...
	# 4: apply any users custom filters
	$email = apply_filters('sph_save_email_filter', $email);
...
function sp_filter_url_save($url) {
	...
	$url = apply_filters('sph_save_url_filter', $url);
...
function sp_filter_filename_save($filename) {
	...
	# 2: apply any users custom filters
	$filename = apply_filters('sph_save_filename_filter', $filename);
...
function sp_filter_save_codetags1($content, $editor, $action) {
	...
	$content = apply_filters('sph_save_codetags1_filter', $content);
...
function sp_filter_save_codetags2($content, $editor, $action) {
	...
	$content = apply_filters('sph_save_codetags2_filter', $content);
...
function sp_filter_save_kses($content) {
	...
	$content = apply_filters('sph_save_kses_filter', $content);
...
function sp_filter_save_linebreaks($content) {
	...
	$content = apply_filters('sph_save_linebreaks_filter', $content);
...
function sp_filter_save_quotes($content) {
	...
	$content = apply_filters('sph_save_quotes_filter', $content);
...
function sp_filter_save_balancetags($content) {
	...
	$content = apply_filters('sph_save_balancetags_filter', $content);
	return $content;
}
...
function sp_filter_save_nofollow($content) {
	...
	$content = apply_filters('sph_save_nofollow_filter', $content);
	return $content;
}
...
function sp_filter_save_links($content, $charcount) {
	...
	$content = apply_filters('sph_save_links_filter', $content);
...
function sp_filter_save_nohtml($content) {
	...
	$content = apply_filters('sph_save_nohtml_filter', $content);
	return $content;
}
...
function sp_filter_save_brackets($content) {
	$content = str_replace('[', '&#091;', $content);
	$content = str_replace(']', '&#093;', $content);
	$content = apply_filters('sph_save_brackets_filter', $content);
	return $content;
}
...
function sp_filter_save_escape($content) {
	...
	$content = apply_filters('sph_save_escape_filter', $content);
	return $content;
}
...
function sp_filter_save_encode($content) {
	...
	$content = apply_filters('sph_save_encode_filter', $content);
	return $content;
}
...
function sp_filter_save_spoiler($content) {
	...
	$content = apply_filters('sph_save_spoiler_filter', $content);
...
function sp_filter_save_images($content) {
	...
	$content = apply_filters('sph_save_images_filter', $content);
}
...
function sp_filter_content_edit($content) {
	...
	if (function_exists('sp_editor_save_linebreaks')) {
		...
		$content = $content = sp_filter_save_linebreaks($content);
	}
	...
	$content = apply_filters('sph_edit_content_filter', $content);
...
function sp_filter_text_edit($content) {
	...
	$content = apply_filters('sph_edit_text_filter', $content);
...
function sp_filter_edit_parser($content, $editor) {
	...
	$content = apply_filters('sph_edit_parser_filter', $content);
	return $content;
}
...
function sp_filter_content_display($content) {
	...
	# 13: apply any users custom filters
	$content = apply_filters('sph_display_post_content_filter', $content);
...
function sp_filter_text_display($content) {
	...
	$content = apply_filters('sph_display_text_filter', $content);
	return $content;
}
...
function sp_filter_title_display($content) {
	...
	$content = apply_filters('sph_display_title_filter', $content);
	return $content;
}
...
function sp_filter_email_display($email) {
	...
	$email = apply_filters('sph_display_email_filter', $email);
	return $email;
}
...
function sp_filter_url_display($url) {
	...
	$url = apply_filters('sph_display_url_filter', $url);
	return $url;
}
...
function sp_filter_display_links($content) {
	$content = sp_make_clickable($content);
	$content = apply_filters('sph_display_links_filter', $content);
	return $content;
}
...
function sp_filter_display_smileys($content) {
	...
	$content = apply_filters('sph_display_smileys_filter', $content);
	return $content;
}
...
function sp_filter_display_chars($content) {
	...
	$content = str_replace(chr(003), "'", $content);
	$content = apply_filters('sph_display_chars_filter', $content);
	return $content;
}
...
function sp_filter_display_paragraphs($content) {
	...
	$content = apply_filters('sph_display_paragraphs_filter', $content);
	return $content;
}
...
function sp_filter_display_codeselect($content) {
	...
	$content = apply_filters('sph_display_codeselect_filter', $content);
...
function sp_filter_display_shortcodes($content) {
	...
	$content = apply_filters('sph_display_shortcodes_filter', $content);
	return $content;
}
...
function sp_filter_display_hidelinks($content) {
	...
	$content = apply_filters('sph_display_hidelinks_filter', $content);
	return $content;
}
...
function sp_filter_signature_display($content) {
	...
	$content = apply_filters('sph_display_signature_filter', $content);
	return $content;
}
...
function sp_filter_tooltip_display($content, $status) {
	...
	if (!$spThisUser->moderator && $status == 1) {
		...
		$content = esc_attr(sp_make_clickable($content));
	}

	$content = apply_filters('sph_display_tooltip_filter', $content);
	return $content;
}
...
function sp_filter_rss_display($content) {
	...
	# 7: apply any users custom filters
	$content = apply_filters('sph_display_rss_content_filter', $content);
...
function sp_filter_display_spoiler($atts, $content) {
	...
	$out = apply_filters('sph_display_spoiler_filter', $out);
