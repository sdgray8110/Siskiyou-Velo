...
function sp_load_current_user() {
    ...
    global $current_user, $spThisUser;
	$spThisUser = new spUser($current_user->ID, true);
}
...
function sp_update_member_data($userid) {
	...
	if ($options['namesync']) {
		$display_name = spdb_table(SFUSERS, "ID=$userid", 'display_name');
...
function sp_delete_member_data($userid) {
	...
	$user_email = spdb_table(SFUSERS, "ID=$userid", 'user_email');

	# 2: get the users display name from members table
	$display_name = sp_get_member_item($userid, 'display_name');
